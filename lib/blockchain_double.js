    ...
    self.latestBlock(function(err, block) {
      ...
      if (options.debug === true) {
        ...
        self.vm.on("step", function(info) {
          var contract = new Account(info.account);
          contract.getCode(self.stateTrie, (err, code) => {
            if (err) {
              console.log(err);
            } else {
              self.logger.log(`pc: ${info.pc}`);
              self.logger.log(`address: ${info.address.toString("hex")}`);
              self.logger.log(`codehash: ${JSON.parse(JSON.stringify(info.account))[3]}`);
              self.logger.log(`bytecode: ${code.toString("hex")}`);
            }
          });
